...
namespace Rsbc.Dmf.BcMailAdapter.Services
{
    ...
    public class BcMailService
    {
        ...
        /// <returns></returns>
        public async Task<ResultStatusReply> SendDocumentsToBcMail(EmptyRequest request, ServerCallContext context)
        {
            var result = new ResultStatusReply();

            var sfegUtils = new SfegUtils(_caseManagerClient, _documentStorageAdapterClient);
