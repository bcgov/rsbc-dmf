...
public class DmerController extends AbstractController {
	...
	public ResponseEntity<?> createDmer(@RequestBody String dmerJson) {
		...
		try {
			...
			licenseNumber = JsonPath.read(JsonPath.read(dmerJson, query).toString(), "$[0].answer[0].valueDecimal")
					.toString();
		} catch (PathNotFoundException e) {
			// Treat same as if path found but element is empty
		}

		// TODO: Need to apply any other criteria for a valid DL number (e.g. 7 digits
		// only ) ?
